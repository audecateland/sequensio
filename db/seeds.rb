...
#sessions
sessions_seed = [{ name: "run", category: "sport" }, { name: "yoga", category: "sport" }, { name: "pasta", category: "cuisine" }, { name: "concentration", category: "travail" }, { name: "balade", category: "promenade" }]

# sequences
seq_un = { name: 'un', duration: '5', playlist_source_id: 'a' }
seq_deux = { name: 'deux', duration: '10', playlist_source_id: 'b', session_id: 2 }
seq_trois = { name: 'trois', duration: '6', playlist_source_id: 'c', session_id: 3 }
seq_quatre = { name: 'quatre', duration: '8', playlist_source_id: 'd', session_id: 4 }
...
users.each do |user|
  ...
  new_session = user.sessions.create!(sessions_seed.sample)
  p "created session: #{new_session}"
  p "generating sequences"
  [seq_un, seq_deux].each do |attributes|
    new_sequence = new_session.sequences.create!(attributes)
